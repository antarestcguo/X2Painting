...
class StableDiffusionXLImg2ImgPanoramaPipeline(StableDiffusionXLImg2ImgPipeline):
    @torch.no_grad()
    # @replace_example_docstring(EXAMPLE_DOC_STRING)
    def gen_multidiffusion(
            self,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for j,(h_start, h_end, w_start, w_end) in enumerate(views):
                    # batch_view = views_batch[0]
                    # h_start, h_end, w_start, w_end = batch_view[0]
                    ...
                    latent_model_input = torch.cat(
                        [latents_for_view] * 2) if self.do_classifier_free_guidance else latents_for_view
                    ...
                    # latents[:, :, h_start:h_end, w_start:w_end] = latents_denoised_batch
                ...
                #     latent_model_input = latents_for_view.repeat_interleave(2, dim=0)if self.do_classifier_free_guidance else latents_for_view
                #     latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
                #
